...
then
    ...
    if [ $out4 -eq 0 ]
    then
        echo "\e[1m\e[33mValidators don't need to outbound. No problem. \e[0m"
    else
        echo "\e[1m\e[33mok. \e[0m"
    fi
fi
...
echo "================================"
echo 'rate_now : \e[1m\e[33m'$v7'%\e[0m  should be >=25% at the end of the test period.'
echo "================================"
echo ""
...
then
    if [ $count -gt 4 ]
    then
        echo "\e[1m\e[33mDone! Check result's so amazing! \e[0m"
    else
        echo "\e[1m\e[33mDone! Check result's good. \e[0m"
    fi
else
